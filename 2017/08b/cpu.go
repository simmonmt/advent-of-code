...
func main() {
	...
	for {
		...
		newVal := regFile[changeReg] + dir*changeVal
		if len(regFile) == 0 {
			maxEverRegVal = newVal
			fmt.Printf("max ever now %v\n", maxEverRegVal)
		} else if newVal > maxEverRegVal {
			maxEverRegVal = newVal
			fmt.Printf("max ever now %v\n", maxEverRegVal)
		}

		regFile[changeReg] = newVal
		//fmt.Printf("now %v\n", regFile[changeReg])
	}

	fmt.Printf("max in file: %v\n", maxReg(&regFile))
	fmt.Printf("max ever in reg: %v\n", maxEverRegVal)
}
