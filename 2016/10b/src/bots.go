...
func pushOutput(outputNum, val int, outputs map[int][]int) {
	if _, found := outputs[outputNum]; !found {
		outputs[outputNum] = []int{}
	}
	outputs[outputNum] = append(outputs[outputNum], val)
}

func pushBotValues(bot *Bot, bots map[int]*Bot, outputs map[int][]int) {
	...
	if bot.outHighDest == DEST_BOT {
		...
		pushOutput(bot.outHighNum, high, outputs)
	}
	...
	if bot.outLowDest == DEST_BOT {
		...
		pushOutput(bot.outLowNum, low, outputs)
	}
}

func populateBots(bots map[int]*Bot, outputs map[int][]int) bool {
	...
	for roundNum := 0; ; roundNum++ {
		for _, bot := range bots {
			...
			pushBotValues(bot, bots, outputs)
...
func main() {
	...
	outputs := map[int][]int{}
	populated := populateBots(bots, outputs)
