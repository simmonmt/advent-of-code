...
func TestRunMaze(t *testing.T) {
	...
	testCases := []TestCase{
		TestCase{4, 4, "hijkl", false, 0},
		TestCase{4, 4, "ihgpwlah", true, 370},
		TestCase{4, 4, "kglvqrro", true, 492},
		TestCase{4, 4, "ulqzkmiv", true, 830},
	}

	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			found, longestPath := RunMaze(tc.w, tc.h, tc.passcode)
			if found != tc.success || (found && len(longestPath) != tc.longestLen) {
				wantResult := "_"
				if tc.success {
					wantResult = strconv.Itoa(tc.longestLen)
				}

				t.Errorf(`RunMaze(%v,%v,"%v") = %v,%v, want %v, %v`,
					tc.w, tc.h, tc.passcode, found, len(longestPath), tc.success, wantResult)
			}
		})
	}
}
