...
func TestRotate(t *testing.T) {
	...
	testTestCases(t, []TestCase{
		TestCase{newRotate(true, 1), "bcdefa", true, "abcdef"},
		TestCase{newRotate(true, 3), "defabc", true, "abcdef"},
		TestCase{newRotate(true, 21), "defabc", true, "abcdef"},

		TestCase{newRotate(false, 1), "fabcde", true, "abcdef"},
		TestCase{newRotate(false, 3), "defabc", true, "abcdef"},
		TestCase{newRotate(false, 21), "defabc", true, "abcdef"},
	})
}
...
func TestRotateMagic(t *testing.T) {
	...
	testTestCases(t, []TestCase{
		TestCase{newRotateMagic('c'), "defabc", true, "abcdef"},
		TestCase{newRotateMagic('d'), "cdefab", true, "abcdef"},
		TestCase{newRotateMagic('b'), "ecabd", true, "abdec"}, // from sample
		TestCase{newRotateMagic('d'), "decab", true, "ecabd"}, // from sample
	})
}
...
func TestMove(t *testing.T) {
	...
	testTestCases(t, []TestCase{
		TestCase{newMove(1, 4), "bdeac", true, "bcdea"}, // from sample
		TestCase{newMove(3, 0), "abdec", true, "bdeac"}, // from sample
	})
}
